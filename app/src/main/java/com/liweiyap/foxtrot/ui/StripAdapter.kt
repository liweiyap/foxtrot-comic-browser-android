...
class StripAdapter(private var database: List<StripDataModel>): RecyclerView.Adapter<StripViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = StripViewHolder(parent)

    override fun onBindViewHolder(holder: StripViewHolder, position: Int) = holder.bind(database[position])
